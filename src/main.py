...
def parse_args(argv):
    ...
    parser.add_argument(
        ...
        help = 'Initial Features Percentage. Default: 0.05')
    parser.add_argument(
        '-pi', '--percent-increment', type = float, default = 0.05,
        help = 'Percentage to Increment Number of Features. Default: 0.05')
    parser.add_argument(
...
if __name__=="__main__":
    ...
    if 'SHA256' in dataset.columns:
        dataset.drop(columns = ['SHA256'], inplace = True)
    if dataset is None:
        print_message('Error Loading Dataset','error', logger)
    try:
