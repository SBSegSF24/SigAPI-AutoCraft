...
def execute_method(method_id, method_dict, features, target, k, logger):
    """
    Executes a feature selection method.

    Parameters:
    method (dict): Dictionary containing the function and information about the method.
    features (pd.DataFrame): Input dataset.
    target (pd.Series): Target variable.
    k (int): Number of features to select.

    Returns:
    str, pd.DataFrame: Name of method and a DataFrame with selected features.
    """
    function = method_dict['function']
    print_message(f'Running {method_id}', 'info', logger)
    results = function(features, target, k)
    return results

def execute_methods_in_parallel(features, target, k, logger):
    """
    Executes all methods in parallel.

    Parameters:
    methods (dict): Dictionary with the methods to be executed.
    features (pd.DataFrame): Input dataset.
    target (pd.Series): Target variable.
    k (int): Number of features to select.

    Returns:
    dict: Results of methods.
    """
    results = dict()
    with concurrent.futures.ThreadPoolExecutor() as executor:
        future_to_method = {
            executor.submit(execute_method, method_id, method_dict, features, target, k, logger): method_id
            for method_id, method_dict in methods.items()
        }
        for future in concurrent.futures.as_completed(future_to_method):
            method_id = future_to_method[future]
            try:
                results[method_id] = future.result()
            except Exception as e:
                print_message(f'Error Executing Method: {method_id}: {e}', 'error', logger)
    return results
...
def calculateMutualInformationGain(features, target, k):
    ...
    print('Finished mutualInformation')
    return df[:k]
...
def calculateSelectKBest(features, target, k):
    ...
    print('Finished selectKBest')
    return df[:k]

methods = {
    'mutualInformation': {'function': calculateMutualInformationGain, 'results': list(), 'is_stable': False},
    'selectRandom': {'function': calculateRandomForestClassifier, 'results': list(), 'is_stable': False},
    'selectExtra': {'function': calculateExtraTreesClassifier, 'results': list(), 'is_stable': False},
    #'RFERandom': {'function': calculateRFERandomForestClassifier, 'results': list(), 'is_stable': False},
    #'RFEGradient': {'function': calculateRFEGradientBoostingClassifier, 'results': list(), 'is_stable': False},
    'selectKBest': {'function': calculateSelectKBest, 'results': list(), 'is_stable': False}
}
