...
def parse_args(argv):
    ...
    parser.add_argument('-met', '--metrics-output-file', type=str, required=True,
        help='File path to save the metrics.')
...
def calculateRF(dataset):
    """
    Calcula as métricas de desempenho de um classificador Random Forest para um conjunto de dados fornecido. 

    A função divide o conjunto de dados em conjuntos de treinamento e teste, treina um classificador Random Forest com os dados de treinamento e avalia o desempenho do classificador nos dados de teste. As métricas de desempenho calculadas são acurácia, precisão, recall e F1-score.

    Parâmetros:
    - dataset (pandas.DataFrame): DataFrame contendo o conjunto de dados. A última coluna é considerada como a variável alvo (classe), e todas as colunas anteriores são consideradas características (features).

    Retorna:
    - metricasRF: Lista contendo as métricas de desempenho do classificador Random Forest. A lista possui quatro valores:
        - Accuracy : Acurácia do classificador.
        - Precision : Precisão do classificador.
        - Recall : Recall do classificador.
        - F1 Score : F1-score do classificador.
    """    
    X = dataset.iloc[:,:-1] # features
    y = dataset.iloc[:,-1] # class

    new_X_train,new_X_test, y_train, y_test = train_test_split(X, y, test_size = 0.3,random_state = 0)
    ...
    data = [{'Accuracy':accuracy,'Precision':precision,'Recall':recall,'F1_Score':f1}]
    df = pd.DataFrame(data)
    print(df)
    metricasRF = [accuracy,precision,recall,f1]
    return metricasRF
...
def selection_phase_Ok(X, y, methods, num_features=1, increment=1):
    """
    Realiza a fase de seleção de características usando diferentes métodos e avalia o desempenho dos métodos ao longo do processo de seleção. 

    A função ajusta o número de características e avalia a estabilidade dos métodos de seleção de características com base nas métricas de desempenho (acurácia, precisão, recall e F1-score). Se um método estável é encontrado, é selecionado como o melhor; caso contrário, um método é escolhido aleatoriamente.

    Parâmetros:
    - X : DataFrame contendo as características do conjunto de dados.
    - y : Série contendo os rótulos de classe ou variáveis alvo associadas às amostras em `X`.
    - methods : Dicionário onde as chaves são nomes de métodos e os valores são dicionários contendo uma função de seleção de características e uma matriz de resultados.
    - num_features : Número inicial de características para começar a seleção. Valor padrão: é 1.
    - increment : Valor para incrementar o número de características em cada iteração. Valor padrão: é 1.

    Retorna:
        - best_stable_method : Nome do melhor método estável encontrado.
        - k : Número de características selecionadas pelo melhor método.
    """
    ...
    print("Num Features: ", total_features)
    while num_features < (total_features + increment) and not has_found_stable_method:
        ...
        for method_name in methods.keys():
            ...
            if(len(methods[method_name]['results']) > 50 and is_method_stable(previous_metrics, current_metrics, parsed_args.threshold)):
    ...
    # Gerar gráficos para cada método
    for method_name in methods.keys():
        plt.figure()
        plt.plot(metrics_history[method_name]['features'], metrics_history[method_name]['accuracy'], label='Acurácia')
        plt.plot(metrics_history[method_name]['features'], metrics_history[method_name]['precision'], label='Precisão')
        plt.plot(metrics_history[method_name]['features'], metrics_history[method_name]['recall'], label='Recall')
        plt.plot(metrics_history[method_name]['features'], metrics_history[method_name]['f1'], label='F1')

        plt.xlabel('Número de features')
        plt.ylabel('Métricas')
        plt.title(f'Desempenho de {method_name} ao longo do processo de seleção')
        plt.legend()
        plt.show()
...
def selection_phase(X, y, methods, num_features=1, increment=1):
    ...
    while num_features < (total_features + increment) and not has_found_stable_method:
        ...
        for method_name in methods.keys():
            ...
            print("Metodos: ", method_name, "Len: ", len(methods[method_name]['results']), "Results: ",  methods[method_name]['results'])
            if len(methods[method_name]['results']) > 100 and is_method_stable(previous_metrics, current_metrics, parsed_args.threshold):
                has_found_stable_method = True
                accuracy = current_metrics[0]
                if accuracy > best_metric_value:
                    best_metric_value = accuracy
    ...
    # Salvar histórico de métricas em um arquivo .csv
    metrics_df = pd.DataFrame()
    for method_name in metrics_history.keys():
        method_df = pd.DataFrame(metrics_history[method_name])
        method_df['method'] = method_name
        metrics_df = pd.concat([metrics_df, method_df])

    metrics_df.to_csv(parsed_args.metrics_output_file, index=False)
